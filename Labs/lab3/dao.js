...
// Get the id of the last film in the database
const getLastId = function(){
    const last_id = new Promise((resolve, reject) => {
        db.get(`SELECT MAX(id) as last_id FROM films`, (err, row) => {
            if (err) reject(err);
...
const updateFilm = function(f){
    return new Promise((resolve, reject) => {
        ...
        db.get(`SELECT * FROM films WHERE id = ?`, [f.id], (err, row) => {
            ...
            const sql = `UPDATE films SET title = ?, watchDate = ?, rating = ?, isFavorite = ?, userId = ?`;
            const date = f.date.format('YYYY-MM-DD');
            db.run(sql, [f.title, date, f.rating, f.favorite, f.user], (err) => {
                if (err) reject(err);
