...
export function FilmLibrary(...array_of_films){
    ...
    this.addNewFilm = function(film){
        if (this.films.some(f => f.id === film.id) || film.id === undefined) {
            let biggest = 0;
            this.films.forEach(f => {
                if (f.id > biggest) {biggest = f.id;}
            });
            film.id = biggest + 1;
        }
    ...
    this.changeFilm = function(id, title, date, rating, favorite, user){
        let film = this.films.find(film => film.id === id);
        if (film === undefined) {throw new Error("Film not found");}
        if (title !== undefined) {film.title = title;}
        if (date !== undefined) {film.date = date;}
        if (rating !== undefined) {film.rating = rating;}
        if (favorite !== undefined) {film.favorite = favorite;}
        if (user !== undefined) {film.user = user;}
    }
}
